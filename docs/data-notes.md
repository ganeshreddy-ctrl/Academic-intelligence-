...
| `content_all` | one row per content unit | **The single inventory of ALL content, both systems.** Columns: course, kind, unit_id, source (`catalogue` = older tables, `ingested` = course_content). **Use this for "what content exists", "which courses have content", "does course X have content/readings".** Do not answer content-coverage from `content_units` or `reading_materials` alone — they miss the ingested courses. |
