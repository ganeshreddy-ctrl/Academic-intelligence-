...
| `student_performance` | one row per **(college, semester, section, course)** | **MCQ & coding practice outcomes** — attendance, attempts, accuracy, completion — 15 colleges, Sem 1-2, 22 `subject`s, 27 `course_id`s. Rates are per-row; **aggregate via the rollups and recompute from summed counts, never average**. Carries canonical `institute_name` + `subject` + `course_id` (built by `build_student_performance.py`; institute via a curated crosswalk). Section is `S-0NN`. |
| `student_perf_by_subject` | one row per college × semester × **subject** | **Use this for per-subject MCQ/coding** (the 22 subject names) — summed attempts/correct/completions + recomputed rates; `*_attendance_pct` is a practice-weighted mean. |
